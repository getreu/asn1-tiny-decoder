...
link:asn1tinydecoder.py[`asn1tinydecoder.py`]
is a simple and fast ASN.1 decoder without external libraries designed 
to parse large files.

There is a good 
http://pyasn1.sourceforge.net/["`Pyasn1`" ASN.1 python library]
out there. It is very complete, but learning how to use it is time
...
to be as fast as possible (with Python 2). 
footnote:[Python 3 would have been a better choice for `asn1tinedecoder`
because of its `bytes` type. Any volunteer to port towards Python 3?]
...
Html and Pdf renditions of this document can be found on 
http://getreu.net/public/downloads/software/ASN1_decoder/[getreu.net].
...
In link:asn1tinydecoder.py[`asn1tinydecoder.py`] a "`node`" is a pointer to an 
...
NOTE: All functions of link:asn1tinydecoder.py[`asn1tinydecoder.py`] are stateless.  
...
`asn1_node_first_child()`:: Opens a ASN1 _sequence_ or _set_ and points 
...
It returns `true` if one ASN1 chunk is inside another chunk.
...
Download `asn1tinydecoder.py` link:asn1tinydecoder.py[here].


.ASN1 decoder source code
[source,python]
----
include::asn1tinydecoder.py[lines=8..-1]
----
...
http://pyasn1.sourceforge.net/[ASN1 python library] 
...
link:search_serial_in_large_CRL.py[`search_serial_in_large_CRL.py`]
searches serials in large CRLs.

It runs through large ASN1 structures creating an index
...
[source,python]
----
include::search_serial_in_large_CRL.py[lines=8..-1]
----
...
----
include::search_serial_in_large_CRL-output.txt[]
----
